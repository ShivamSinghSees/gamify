    ...
    if (isMobile) {
      return (
        <div
          ...
          data-mobile="true"
          className={cn(
            'fixed inset-y-0 z-50 flex h-svh w-[--sidebar-width-mobile] flex-col bg-sidebar text-sidebar-foreground transition-transform duration-200 ease-linear',
            side === 'left'
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="group"
      className={cn('relative flex w-full min-w-0 flex-col p-4 pt-2', className)}
      {...props}
    />
  )
})
